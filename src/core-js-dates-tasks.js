...
function getNextFriday(date) {
  ...
  if (!daysToNextFriday) daysToNextFriday = 7;
  const result = new Date(date);
  result.setDate(result.getDate() + daysToNextFriday);
  return result;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const days = (endDate - startDate) / 3600 / 24 / 1000 + 1;
  return days;
}
/**
...
function isDateInPeriod(date, period) {
  ...
  const checkedTime = new Date(date).getTime();
  return checkedTime >= startDate && checkedTime <= endDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const result = [];
  const [startDay, startMonth, statYear] = period.start.split('-');
  const startDate = new Date(`${statYear}-${startMonth}-${startDay}`);
  const [endDay, endMonth, endYear] = period.end.split('-');
  const endDate = new Date(`${endYear}-${endMonth}-${endDay}`);
  const periodDays = (endDate - startDate) / (1000 * 3600 * 24) + 1;
